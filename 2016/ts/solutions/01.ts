...
const line: string = (await getInput(import.meta.url))[0];
...
function processInstruction(string: string) {
  const dir = string[0];
  const num = Number.parseInt(string.slice(1));
...
function getDistance(_x: number, _y: number) {
